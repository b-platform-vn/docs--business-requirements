...
| | |
|---|---|
| **Repo** | `github.com/b-platform-vn/api-service-auth` (planned) |
| **Current name** | `api-auth` → [old doc](./deprecated/api-auth.md) |
| **v3 target** | `api-service-auth` (L2) — rename opportunistically |
| **Layer** | L2 — API Services |
| **Status** | planned (rename of `api-auth`) |
...
- Exposes auth endpoints consumed by [`platform-endpoints`](./platform-endpoints.md) webhook/API routing and by frontends via Server Actions.
...
- `api-auth` is referenced by other MCM connectors' local-dev `dummy-auth-server` (port 3003, `/api/auth/validate`) — confirm whether `api-service-auth` itself implements that contract in production.
